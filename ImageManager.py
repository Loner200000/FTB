...
class ImageManagerMod(loader.Module): 
    ...
    async def invertcmd(self, m): 
        ...
        reply = await m.get_reply_message() 
        im = await get_img_from_msg(reply) 
        if not im: return await m.edit(s+"Реплай на изображение") 
    ...
    async def contrstcmd(self, m): 
        ...
        await m.edit(s+"Работаю...") 
        args = utils.get_args_raw(m)         
        if not args: return await m.edit(s+"Укажите <level:float or int>") 
        rr = re.compile(r"^(\d+|\d+\.\d+)$") 
        if not rr.match(args): return await m.edit(s+"Укажите <level:float or int>") 
    ...
    async def brightcmd(self, m): 
        ...
        args = utils.get_args_raw(m)         
        if not args: 
            return await m.edit(s+"Укажите <level:float or int>") 
        rr = re.compile(r"^(\d+|\d+\.\d+)$") 
        if not rr.match(args): 
            return await m.edit(s+"Укажите <level:float or int>") 
        c = float(rr.findall(args)[0]) 
